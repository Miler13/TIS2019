...
class PDF extends FPDF
{
		function Header()
		{
		   ...
		   $Turno = $_POST['turno'];
            $asignacion_aux = mysqli_query($conexion,"SELECT NombreTurno FROM turnos where idTurno = '$Turno'");
            while($row = mysqli_fetch_row($asignacion_aux)){
            $NombreTurno = $row[0];

            }
		     $this->Cell(90,20, $Turno, 0,0,'C');
			$this->Ln(15);
