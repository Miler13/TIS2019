...
class PDF extends FPDF
{
		function Header()
		{
		    ...
		    $Horario = $_POST['horario'];
            $asignacion_aux = mysqli_query($conexion,"SELECT NombreHorario FROM horarios where idHorario = '$Horario'");
            while($row = mysqli_fetch_row($asignacion_aux)){
            $NombreHorario = $row[0];

            }
		     $this->Cell(110,20, $Horario, 0,0,'C');
			$this->Ln(15);
        ...
        $item = 0;
			while($asignacion_aux2 = mysqli_fetch_array($asignacion_aux)){
			    ...
			    $pdf->Cell(15, 8, $asignacion_aux2['Grupo'], 0,'C');
			   	$pdf->Cell(30, 8, $asignacion_aux2['Turno'], 0,'C');
