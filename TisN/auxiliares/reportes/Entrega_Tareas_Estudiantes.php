...
class PDF extends FPDF
{
		...
		$asignacion_aux = mysqli_query($conexion,"SELECT concat(estudiantes.NombresEstudiante,' ',estudiantes.ApellidosEstudiante) as Estudiante, asignaturas.NombreAsignatura as Asignatura, entrega_tareas.Descripcion as Descripcion, entrega_tareas.Archivo as Archivo, entrega_tareas.FechaEntrega as Fecha FROM asignacion_aux INNER JOIN asignaturas ON  asignacion_aux.idAsignatura =  asignaturas.idAsignatura 
...
where asignacion_aux.idDocente = '$codigo' and asignaturas.NombreAsignatura = '$Asignatura' group by estudiantes.idEstudiante ");
        ...
        $item = 0;
			while($asignacion_aux2 = mysqli_fetch_array($asignacion_aux)){
				...
				$pdf->Cell(40, 8, utf8_decode($asignacion_aux2['Estudiante']), 0,'C');
				$pdf->Cell(50, 8, utf8_decode($asignacion_aux2['Descripcion']), 0,'C');
				$pdf->Cell(50, 8, $asignacion_aux2['Archivo'], 0,'C');
			    $pdf->Cell(40, 8, $asignacion_aux2['Fecha'], 0,'C');
				$pdf->Ln(5);
			}
