...
class PDF extends FPDF
{
		function Header()
		{
		    ...
		    $Asignatura = $_POST['asignatura'];
		            $asignacion_aux = mysqli_query($conexion,"SELECT NombreAsignatura FROM asignaturas where idAsignatura = '$Asignatura'");
		            while($row = mysqli_fetch_row($asignacion_aux)){
		            $NombreAsignatura = $row[0];

            }
		     $this->Cell(95,20, $Asignatura, 0,0,'R');
			$this->Ln(15);
			...
			$this->Cell(40, 8, 'Auxiliar' ,1,0,'C');//dicente
       ...
       where asignaturas.idAsignatura  = '$Asignatura' ");
        ...
        $item = 0;
			while($asignacion_aux2 = mysqli_fetch_array($asignacion_aux)){
				...
				$pdf->Cell(40, 8, utf8_decode($asignacion_aux2['Docente']), 0,'C');
