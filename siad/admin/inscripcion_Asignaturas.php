...
if(isset($_SESSION['NombreUsuario'])) {
     if ($_SESSION["NivelUsuario"] == 1) {
            $user = $_SESSION['NombreUsuario'];
              ...
              $consulta=mysqli_query("select Foto from usuarios where Codigo = $codigo");                  
        ...
        ?>
          ...
          $carrera=mysqli_query($consulta1);
          $consulta2="select idAsignatura, NombreAsignatura from asignaturas";
          $asignaturas=mysqli_query($consulta2);
             ...
             $consulta=mysqli_query("select Foto from estudiantes where idEstudiante = $codigo");                  
                while($filas=mysqli_fetch_array($consulta)){
                 ...
                 $consulta2 = mysqli_query("select concat (NombresEstudiante, ' ', ApellidosEstudiante) as Estudiante from estudiantes where idEstudiante = $codigo"); 
...
$registro = mysqli_query("SELECT inscripciones_asignaturas.idInscripcion as Codigo, carreras.NombreCarrera as Carrera, asignaturas.NombreAsignatura as Asignatura, concat(estudiantes.NombresEstudiante, ' ' ,estudiantes.ApellidosEstudiante) as Estudiantes,
...
?>
    ...
    <?php
include('conexion.php');
    $numeroRegistros = mysqli_num_rows(mysqli_query("SELECT * FROM inscripciones_asignaturas"));
