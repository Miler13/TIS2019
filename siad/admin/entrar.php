...
<?php
		include('conexion.php');
			...
			$username=htmlentities(mysqli_real_escape_string($_POST['usuario']));
			$password=mysqli_real_escape_string($_POST['password']);
			$tipo = $_POST['tipousuario'];
				
				$query = mysqli_query("SELECT Nombreusuario,PassUsuario, NivelUsuario FROM usuarios WHERE Nombreusuario = '$username'") or die(mysqli_error());
				$data = mysqli_fetch_array($query);
			        if($data['Nombreusuario'] != $username && ['PassUsuario'] != $password)  {
		             ...
		             else
		              {
							...
							//$query = mysqli_query("SELECT username,password,imagen FROM usuarios WHERE username = '$username'") or die(mysqli_error());
