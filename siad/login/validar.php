...
<?php session_start(); ?>
		...
		<?php
			...
			if(isset($_POST['usuario'])){
				...
				$usuario = htmlentities(mysqli_real_escape_string($_POST['usuario']));
				$pw = htmlentities(mysqli_real_escape_string($_POST['password']));

			//	$numero = srand((double)microtime()*1000000);

				$log = mysqli_query("SELECT * FROM usuarios WHERE NombreUsuario='$usuario' AND PassUsuario='$pw'");
